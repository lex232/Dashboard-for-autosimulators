...
if __name__ == '__main__':
    ...
    while True:
        temp_dict = assettoReader.getData()
        speed = int(temp_dict.get("speed"))
        rpm = temp_dict.get("rpm")
        gear = temp_dict.get("gear")
        tc = temp_dict.get("tc1")
        tcinaction = temp_dict.get("tcinAction")
        output = str(speed) + ":" + str(rpm) + ':' + str(gear) + ':' + str(tc)
        output_hex = output.encode('utf_8')
        # print('Assetto data:', speed, rpm, gear, tc, tcinaction)
        ser.write(output_hex)
        time.sleep(0.1)
